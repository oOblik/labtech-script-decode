 ...
 * @private
 */
const {Functions, RemoteCommands} = require('./constants');
...
function getFunctionLineDescription(step) {
  ...
  return functionTemplates[step.FunctionId](step.Param1, step.Param2, step.Param3, step.Param4, step.Param5, step.Sort + 1);
}
...
functionTemplates[2] = (p1, p2) => `LTCommand: ${RemoteCommands[p1]} ${p2}`;
...
functionTemplates[32] = (p1) => `Set Default Printer: ${p1}`;
functionTemplates[33] = (p1) => `Clear Printer Queue: ${p1}`;
...
functionTemplates[70] = (p1, p2, p3, p4, _5, line) => {
  let temp = p1;
  ...
  if (p4 === '' || p4 === 0) {
    return `IF ${temp} ${compare} ${p3} THEN Exit Script`;
  }
  if (!Number.isInteger(p4) && p4 !== '') {
    return `IF ${temp} ${compare} ${p3} THEN Jump to ${p4}`;
  }
  return `IF ${temp} ${compare} ${p3} THEN Jump to line ${line + 1 + p4}`;
};
...
functionTemplates[199] = (_1, p2, p3) => `SET: @${('' + p3).replace(/@/g, '')}@ = Script Stat (${p2})`;
...
functionTemplates[217] = (p1, p2) => `SET: @${('' + p2).replace(/@/g, '')}@ = TemplateProperty(${p1})`;
...
functionTemplates[230] = (p1, p2) => `SET: @${('' + p2).replace(/@/g, '')}@ = AttachmentID(${p1})`;
...
functionTemplates[240] = () => `Checking connectivity of target device and storing the result in %pingresult%`;
...
exports.getFunctionLineDescription = getFunctionLineDescription;
